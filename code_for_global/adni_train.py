...
# from model.model_gap_fc import GAN
...
def train(cosine_loss,i_fold,save_path,DEVICE,weight_decay,lr,lr_decay,train_epochs,node_num,feat_dim,hidden_num,class_num,nheads,dropout,bs):
    ...
    for epoch in range(train_epochs):
        ...
        val_loss, val_acc, score_auc, preds, preds_score, targets, risks,val_CE_loss,val_Pos_loss,val_Neg_loss,val_score_pre,val_score_rec,val_score_f1 = val_epoch(cosine_loss,model, ValLoader, DEVICE,epoch,i_fold,save_path)
        ...
        if is_best_score:
            ...
            best_score_pre,best_score_rec,best_score_f1=val_score_pre,val_score_rec,val_score_f1
        # if is_test_best_score:
    ...
    return test_preds_score,best_score_acc, best_score_auc,test_acc,test_auc,test_preds,test_targets,test_score_pre,test_score_rec,test_score_f1,best_score_pre,best_score_rec,best_score_f1
...
if __name__ == '__main__':
    ...
    bs = 128
    cosine_loss = True
    network_name = increment_dir(f'journal_lr{lr:.1e}_lr_decay{lr_decay:.1e}_wd{weight_decay}' \
    ...
    for i_fold in range(1,6):
        test_preds_score,best_score_acc, best_score_auc,test_acc,test_auc,test_preds,test_target,test_score_pre,test_score_rec,test_score_f1,best_score_pre,best_score_rec,best_score_f1 = train(cosine_loss,i_fold,save_path,DEVICE,weight_decay,lr,lr_decay,train_epochs,node_num,feat_dim,hidden_num,class_num,nheads,dropout,bs)
        test_targets.append(test_target)
        test_preds_scores.append(test_preds_score)

        accs.append(best_score_acc)
        aucs.append(best_score_auc)
        pres.append(best_score_pre)
        recs.append(best_score_rec)
        f1s.append(best_score_f1)
    ...
    logging.info(f'val_rec {np.array(recs).mean():.4f}±{np.array(recs).std():.4f}')
    logging.info(f'val_pre {np.array(pres).mean():.4f}±{np.array(pres).std():.4f}')
    
    logging.info(f'val_f1 {np.array(f1s).mean():.4f}±{np.array(f1s).std():.4f}')
