...
class GAN(nn.Module):
    def __init__(self, feat_dim, node_num, hidden_num, class_num, nheads,dropout=0.5): #2
        ...
        self.pooling = nn.AvgPool2d((node_num,1))
    ...
    def forward(self, x,x_neg, adj,adj_neg):
        ...
        # out_neg_0 = self.pooling(x_neg).view(x_neg.size()[0],-1)
        # out_neg_0 = F.tanh(out_neg_0)
        # # x_neg = F.dropout(x_neg, self.dropout, training=self.training)
        ...
        # cos_similarity_0 = torch.cosine_similarity(out_0,out_neg_0,dim=1)
        # cos_similarity = torch.cosine_similarity(out,out_neg,dim=1)
        # cos_similarity = cos_similarity+cos_similarity_0

        ypred = self.linear(out)

        return ypred, torch.zeros(128).float().to('cuda'),torch.zeros(128).float().to('cuda'), out,out_neg
...
if __name__ == "__main__":
    ...
    a,b,c,d,_ = model(x, x_neg,adj,adj_neg)
    print(a.shape)
    # print(b.shape)
    # print(c.shape)
    print(d.shape)
